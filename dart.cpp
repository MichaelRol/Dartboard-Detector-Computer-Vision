...
Mat detectBoards(Mat originalImage, Mat houghSpace, int* buckets, string filename) {
    ...
    // count detected lines in hough space
    ...
    // array holding rho and theta for each line
	...
	// for each line calculate whether it intersects with each of the other lines
	for (int line = 0; line < count; line++) {
		for (int restoflines = 0; restoflines < count; restoflines++) {
		  if (line != restoflines) {
				...
				//use Cramer's Rule to determine where two lines cross
				int x = round((r0*b1 - b0*r1)/(a0*b1 - b0*a1));

				int y = round((a0*r1 - r0*a1)/(a0*b1 - b0*a1));

				// for each bucket recalculate average intersection point within the bucket
				for(int bucketx = 0; bucketx < 20; bucketx++) {
					for (int buckety = 0; buckety < 20; buckety++) {
						if (x >= bucketx * bucketsizex && x < (bucketx+1)*bucketsizex && y >= buckety * bucketsizey && y < (buckety+1)*bucketsizey) {
							//number of lines intersecting within the bucket
							buckets[bucketx*20+buckety]++;

							//average x and y values of intersections within the bucket
	...
	// For each bucket check whether there is a detected board within it
	for(int x = 0; x < 20; x++){
		for(int y = 0; y < 20; y++){
			for( int i = 0; i < boards.size(); i++ ) {

				//calculate coords of the corners of the rectangles generated by Viola-Jones
				...
				// check that the average intersection point of the bucket is within the detected board's rectangle
...
Mat generateLineHoughSpace(Mat gradMag, Mat gradDir) {
	...
	Mat output;
	double pi = 3.1415926535897;
    // plot rho and theta for pixels of non zero value creating a hough space image
